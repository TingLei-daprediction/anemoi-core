#!/usr/bin/env bash
# Run recursive +3h verification for base_4 checkpoints (refc weight=5,
# num_layers=8, refc-input, no-hydrometeors, base_1 normalization, finer_graph_v1).
set -euo pipefail
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-3h-refc-value-base_4-refc-input-no-hydrometeors-finer-graph-v1-single-input"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-3h-refc-value-base_4-refc-input-no-hydrometeors-finer-graph-v1-single-input.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_VERIFY_ROOT: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/base_4_graphtransformer_finer_graph_v1_single_input_refc_input_no_hydrometeors/verify_3h/"
