...
# prognostic/input variable, hydrometeors removed, base_1 normalization, and
# finer_graph_v1.
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_export_3h_refc_value_base_1_refc_input_no_hydrometeors_finer_graph_v1_single_input.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-3h-refc-value-base_1-refc-input-no-hydrometeors-finer-graph-v1-single-input"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-3h-refc-value-base_1-refc-input-no-hydrometeors-finer-graph-v1-single-input.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_VERIFY_ROOT: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/base_1_graphtransformer_finer_graph_v1_single_input_refc_input_no_hydrometeors/verify_3h/"
