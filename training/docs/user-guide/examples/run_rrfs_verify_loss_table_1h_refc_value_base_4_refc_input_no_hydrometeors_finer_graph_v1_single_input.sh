#!/usr/bin/env bash
# Export configured loss tables for base_4 (refc weight=5, num_layers=8,
# refc-input, no-hydrometeors, finer_graph_v1).
set -euo pipefail
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1h-refc-value-base_4-refc-input-no-hydrometeors-finer-graph-v1-single-input-loss-table"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1h-refc-value-base_4-refc-input-no-hydrometeors-finer-graph-v1-single-input-loss-table.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_LOSS_TABLE_DIR: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/base_4_graphtransformer_finer_graph_v1_single_input_refc_input_no_hydrometeors/verify/loss_tables"
