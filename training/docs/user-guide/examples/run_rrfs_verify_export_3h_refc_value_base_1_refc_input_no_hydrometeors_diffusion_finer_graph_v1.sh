#!/usr/bin/env bash
# Run recursive +3h verification for checkpoints trained with refc in the
# normal prognostic/input state, hydrometeors removed, diffusion model path,
# base_1 forcing/static normalization, and finer_graph_v1.
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_export_3h_refc_value_base_1_refc_input_no_hydrometeors_diffusion_finer_graph_v1.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-3h-refc-value-base_1-refc-input-no-hydrometeors-diffusion-finer-graph-v1"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-3h-refc-value-base_1-refc-input-no-hydrometeors-diffusion-finer-graph-v1.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_VERIFY_ROOT: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/base_1_diffusion_finer_graph_v1_refc_input_no_hydrometeors/verify_3h/"
